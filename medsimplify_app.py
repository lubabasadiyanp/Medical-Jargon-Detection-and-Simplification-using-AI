...
MODEL_RESULTS = [
    ...
    {"Model":"Gemini 1.5 Flash (free)","ROUGE-1":0.43,"ROUGE-2":0.21,"ROUGE-L":0.40,"BLEU":0.15,"Notes":"Free API"},
]
...
def simplify_text(text: str) -> str:
    ...
    if client is None:
        return (
            "❌ API key not found.\n\n"
            "• Local: add GEMINI_API_KEY to .streamlit/secrets.toml\n"
            "• Streamlit Cloud: Settings → Secrets\n\n"
            "Get a FREE key at: https://aistudio.google.com/"
        )
    try:
        prompt = (
            "You are a medical assistant. Simplify the following medical text so that "
            "a non-medical person can easily understand it. Use simple everyday words, "
            "avoid jargon, keep the same meaning, and be concise.\n\n"
            f"Medical text: {text}\n\nSimplified version:"
        )
...
[data-testid="stMetric"] { background:#fff; border:1px solid #e2ddd6; border-radius:12px; padding:1rem 1.25rem; }
...
.stTextArea textarea { background:#f8f6f1; border:1px solid #e2ddd6; border-radius:8px; font-size:0.9rem; }
.stButton > button { background:#c8441a !important; color:white !important; border:none !important; border-radius:6px !important; font-weight:600 !important; padding:0.6rem 1.5rem !important; }
.stButton > button:hover { background:#a83615 !important; }
.simplified-box { background:#e4f5f2; border-left:4px solid #1a7a6e; border-radius:8px; padding:1rem 1.25rem; color:#1a5a50; font-size:0.95rem; line-height:1.75; margin-top:0.5rem; }
.reference-box { background:#fffbf5; border-left:4px solid #b87c30; border-radius:8px; padding:1rem 1.25rem; color:#6b5030; font-size:0.9rem; line-height:1.75; }
.eyebrow { font-size:0.72rem; font-weight:600; letter-spacing:0.15em; text-transform:uppercase; color:#c8441a; }
.hero-title { font-family:'DM Serif Display',serif; font-size:2.6rem; line-height:1.15; color:#1a1814; }
</style>
...
with st.sidebar:
    ...
    st.divider()
    st.markdown("**Model:** Gemini 2.0 Flash (Free)")
...
if page == "🏠 Home":
    ...
    st.markdown('<div class="hero-title">Making medical research <span style="color:#c8441a;font-style:italic">readable</span> for everyone</div>', unsafe_allow_html=True)
    st.markdown("A neural text simplification system trained on **921 medical abstract pairs** across **75 clinical topics** — bridging the gap between scientific literature and patient understanding.")
    st.divider()
    c1,c2,c3,c4,c5 = st.columns(5)
    c1.metric("Parallel Pairs","921"); c2.metric("Clinical Topics","75")
    c3.metric("Best ROUGE-1","0.48"); c4.metric("Jargon Annotations","4,511"); c5.metric("Medical Sources","15")
    st.divider()
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("Paste any medical text and get a plain-language version powered by **Gemini AI (Free)**.")
    ...
    st.divider()
    input_text = st.text_area("📝 Input — Medical Text",
        value=st.session_state.get("demo_input",""),
        height=200, placeholder="Paste complex medical text here…")
    ...
    if output:
        ...
        st.download_button("⬇ Download simplified text", data=output,
            file_name="simplified_output.txt", mime="text/plain")
    else:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("A parallel corpus of complex and simplified medical abstracts with jargon annotations and human readability scores.")
    st.divider()
    c1,c2,c3,c4 = st.columns(4)
    c1.metric("Total Pairs","921"); c2.metric("Training Pairs","635 (68.9%)")
    c3.metric("Validation Pairs","138 (15.0%)"); c4.metric("Test Pairs","148 (16.1%)")
    st.divider()
    col1,col2,col3 = st.columns(3)
    with col1: st.info("**📄 Parallel Pairs**\n\n921 matched complex–simple pairs across 75 topics.\n\n`train.csv` · `val.csv` · `test.csv`")
    with col2: st.info("**🔬 Readability Corpus**\n\n4,504 sentences with human scores (1–6 scale) from 15 sources.\n\n`readability.csv`")
    with col3: st.info("**🏷️ Jargon Corpus**\n\n4,511 sentences with NER span annotations for 7 categories.\n\n`jargon.json`")
    st.divider()
    ...
    df_s = pd.DataFrame({"Source":list(sorted_scores.keys()),"Score":list(sorted_scores.values())})
    st.bar_chart(df_s.set_index("Source"), color="#c8441a", height=400)
    st.divider()
    st.subheader("Dataset Split")
    st.dataframe(pd.DataFrame({"Split":["Train","Validation","Test"],"Pairs":[635,138,148],"Percentage":["68.9%","15.0%","16.1%"]}),
        use_container_width=True, hide_index=True)
    st.divider()
    st.subheader("Jargon Annotation Categories")
    jargon_df = pd.DataFrame({"Category":["Hard Medical Terms","Easy Medical Terms","Medical Abbreviations","Complex General","Drug Names","Anatomy Terms","Procedures"],"Count":[1247,934,812,678,423,289,128]})
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("Best results achieved with **BART-base fine-tuned**. Evaluated on **148 held-out test pairs**.")
    st.divider()
    st.subheader("Best Model Metrics — BART-base Fine-tuned")
    c1,c2,c3,c4 = st.columns(4)
    c1.metric("ROUGE-1","0.48"); c2.metric("ROUGE-2","0.24")
    c3.metric("ROUGE-L","0.44"); c4.metric("BLEU","0.18")
    st.divider()
    st.subheader("Training Pipeline")
    p1,p2,p3,p4 = st.columns(4)
    p1.markdown("**01 — Data Prep**\n\nLoad `train.csv`, tokenize with BART tokenizer.")
    p2.markdown("**02 — Jargon Detection**\n\nUse `jargon.json` NER spans as input features.")
    p3.markdown("**03 — Fine-tuning**\n\nSeq2seq on 635 pairs. Early stop on ROUGE-L.")
    p4.markdown("**04 — Evaluation**\n\nScore 148 test pairs with ROUGE & BLEU.")
    st.divider()
    st.subheader("Model Comparison")
    df = pd.DataFrame(MODEL_RESULTS)
    st.dataframe(df.style.highlight_max(subset=["ROUGE-1","ROUGE-2","ROUGE-L","BLEU"], color="#d4edda"),
        use_container_width=True, hide_index=True)
    st.divider()
    st.subheader("ROUGE-1 Score by Model")
    chart_df = pd.DataFrame({"Model":[r["Model"].replace(" ⭐ Best","") for r in MODEL_RESULTS],"ROUGE-1":[r["ROUGE-1"] for r in MODEL_RESULTS]})
...
simplified versions, covering **75 clinical topics** from PubMed.
    """)
    st.divider()
    st.subheader("Technology Stack")
    col1,col2,col3,col4 = st.columns(4)
    col1.markdown("- `Python 3.11`\n- `Streamlit`")
    col2.markdown("- `Google Gemini`\n- `Free API`")
...
| Easiest source | NIHR Public Health (3.61) |
    """)
    st.divider()
    st.subheader("🔑 Free API Key Setup — Google Gemini")
    st.success("✅ Google Gemini API is completely FREE — 1,500 requests/day, no credit card needed")
    st.markdown("**Step 1** — Get your free key at 👉 https://aistudio.google.com/")
    st.markdown("**Step 2 — Local** — create `.streamlit/secrets.toml`:")
    st.code('GEMINI_API_KEY = "your-key-here"', language="toml")
    st.markdown("**Step 3 — Streamlit Cloud** → Settings → Secrets:")
    st.code('GEMINI_API_KEY = "your-key-here"', language="toml")